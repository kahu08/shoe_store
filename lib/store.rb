class Movie
    ...
    define_method(:save) do
      result = DB.exec("INSERT INTO movies (name) VALUES ('[email protected]}') RETURNING id;")
    ...
    define_method(:update) do |attributes|
      ...
      attributes.fetch(:shoe_ids, []).each() do |shoe_id|
        DB.exec("INSERT INTO shoes_movies (shoe_id, movie_id) VALUES (#{shoe_id}, #{self.id()});")
      end
    end

    define_method(:delete) do
        DB.exec("DELETE FROM shoes_movies WHERE movie_id = #{self.id()};")
